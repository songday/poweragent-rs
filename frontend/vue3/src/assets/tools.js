...
export function genBranchesByNode(node) {
  if (!node || !node.ports || node.ports.length == 0) return []
  const branches = []
...
export function checkConditionBranches(branches) {
  for (let bIdx = 0; bIdx < branches.length; bIdx++) {
    ...
    if (!b.branchId) return constructBranchInvalidMessage(bIdx, 'branchId is missing', 0, null)
    if (!b.branchName) return constructBranchInvalidMessage(bIdx, 'branchName is missing')
    if (b.branchType === 'GotoAnotherNode') continue
...
export function l(m) {
  if (!import.meta.env.PROD) console.log(m)
}
...
export function genNodeName(prefix, allNodeNameSet) {
  let name = ''
  let n = ''
  do {
    n = n == null ? Date.now().toString(16) : Math.random().toString(16).substring(2)
    name = prefix + '-' + n
  } while (allNodeNameSet.has(name))
  allNodeNameSet.add(name)
  return name
}

...
import { copyProperties, getDefaultBranch, httpReq, genNodeName } from '@/assets/tools.js'
...
const validate = () => {
  ...
  if (getNode().getPortAt(0).id == '') m.push('Please connect "Next" to another node')
...
const addBranches = () => {
  if (originAsyncReqSetting == nodeData.reqInfo.asyncReq) return
  node.removePorts()
  nodeData.branches = []
...
onMounted(async () => {
  if (nodeData.newNode) {
    ...
    nodeData.nodeName = genNodeName(t('httpReqNode.nodeName'), allNodeNameSet.value)
...
const newParam = () => {
  ...
  if (p == 'h') dynamicTitle.value = t('httpReqNode.dynamicTitle.h')
  else if (p == 'q') dynamicTitle.value = t('httpReqNode.dynamicTitle.q')
  else if (p == 'f') dynamicTitle.value = t('httpReqNode.dynamicTitle.f')
...
const addParam = () => {
  ...
  if (idx > -1) {
    if (activeName.value == 'h') nodeData.reqInfo.headers[idx] = p
    else if (activeName.value == 'q') nodeData.reqInfo.queryParams[idx] = p
    else if (activeName.value == 'f') nodeData.reqInfo.formData[idx] = p
  } else {
    if (activeName.value == 'h') nodeData.reqInfo.headers.push(p)
    else if (activeName.value == 'q') nodeData.reqInfo.queryParams.push(p)
    else if (activeName.value == 'f') nodeData.reqInfo.formData.push(p)
  }
  paramSetFormVisible.value = false
}
const editParam = (idx) => {
  editIdx.value = idx
  if (activeName.value == 'h') copyProperties(nodeData.reqInfo.headers[idx], param)
  else if (activeName.value == 'q') copyProperties(nodeData.reqInfo.queryParams[idx], param)
  else if (activeName.value == 'f') copyProperties(nodeData.reqInfo.formData[idx], param)
  paramSetFormVisible.value = true
}
const delParam = (idx) => {
  if (activeName.value == 'h') nodeData.reqInfo.headers[idx].splice(idx, 1)
  else if (activeName.value == 'q') nodeData.reqInfo.queryParams[idx].splice(idx, 1)
  else if (activeName.value == 'f') nodeData.reqInfo.formData[idx].splice(idx, 1)
}
const insertVar = () => {
...
const changeTab = (v) => {
  if (v != 'POST' && activeName.value == 'f') activeName.value = 'q'
}
</script>
<style scoped>
.nodeBox {
      ...
      <el-form :model="nodeData" label-width="90px">
        ...
        <el-form-item label="Address">
          <el-input v-model="nodeData.reqInfo.address">
            ...
            <template #prepend
              >{{ nodeData.reqInfo.method }} {{ nodeData.reqInfo.protocol }}</template
            >
          </el-input>
        </el-form-item>
          ...
          <el-tabs v-model="activeName" class="demo-tabs" @tab-click="handleClick">
            ...
            <el-tab-pane label="Request body" name="f" v-if="nodeData.reqInfo.method == 'POST'">
              ...
              <el-radio-group v-model="nodeData.reqInfo.postContentType" class="ml-4">
                <el-radio value="UrlEncoded" size="large"
                  >application/x-www-form-urlencoded</el-radio
                >
                <el-radio value="JSON" size="large">JSON</el-radio>
                <!-- <el-radio value="1" size="large">multipart/form-data</el-radio> -->
              </el-radio-group>
              <el-table
                v-if="nodeData.reqInfo.postContentType == 'UrlEncoded'"
                :data="nodeData.reqInfo.formData"
                stripe
                style="width: 100%"
              >
                <el-table-column prop="name" label="Parameter name" width="300" />
              ...
              <el-button
                ...
                v-if="nodeData.reqInfo.postContentType == 'UrlEncoded'"
                @click="newParam"
                >+Add form data</el-button
              >
              <!-- <div style="margin: 20px 0" /> -->
              <el-input
                ref="requestBodyRef"
                v-if="nodeData.reqInfo.postContentType == 'JSON'"
                v-model="nodeData.reqInfo.requestBody"
                maxlength="10240"
                placeholder="JSON"
                show-word-limit
                type="textarea"
        ...
        <el-form-item label="Timeout" :label-width="formLabelWidth">
          <el-input-number
            v-model="nodeData.reqInfo.timeoutMilliseconds"
            :min="200"
            :max="600000"
          />
          {{ $t('common.millis') }}
        ...
        <el-form-item
          v-show="!nodeData.reqInfo.asyncReq"
          label="Status code"
          :label-width="formLabelWidth"
        >
          <el-input
            v-model="nodeData.statusCodeVarName"
            autocomplete="on"
        ...
        >
          <el-input
            v-model="nodeData.responseBodyVarName"
            autocomplete="on"
            placeholder="Enter a variable name"
          />
        </el-form-item>
      ...
      <el-form :model="param">
        ...
        <el-form-item label="Value" :label-width="formLabelWidth">
          <el-space size="default" spacer="-">
            ...
            <el-input
              v-if="param.valueSource == 'Val'"
              v-model="param.value"
              autocomplete="off"
              style="width: 400px"
            />
            <el-select
              v-if="param.valueSource == 'Var'"
              v-model="selectedVar"
              placeholder="Select a varaible"
              style="width: 400px"
            >
